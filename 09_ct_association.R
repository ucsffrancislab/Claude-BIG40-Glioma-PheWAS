...
# 09_ct_association.R
#
# Association testing of C+T polygenic scores against glioma outcomes.
...
#
# Usage:
#   Rscript 09_ct_association.R <ct_output_dir> <input_dir> [n_cores]
#
# Example:
#   Rscript 09_ct_association.R ${PWD}/ct_output ${PWD}/input 32
#
# Inputs:
#   <ct_output_dir>/scores/<cohort>/<IDP>.profile  (from 08b_run_ct.sh)
#   <input_dir>/<cohort>-covariates.csv             (IID, case, age, sex, PCs, ...)
#
# Outputs:
#   <ct_output_dir>/association/ct_association_results.csv   (all results)
#   <ct_output_dir>/association/ct_association_summary.csv   (top hits)
#   <ct_output_dir>/association/ct_meta_results.csv          (meta-analysis)
# =============================================================================
...
if (length(args) < 2) {
    stop("Usage: Rscript 09_ct_association.R <ct_output_dir> <input_dir> [n_cores]")
}
...
score_dir <- file.path(CT_DIR, "scores")
first_cohort <- COHORTS[1]
idp_files <- list.files(file.path(score_dir, first_cohort), pattern = "\\.profile$")
idp_ids <- sub("\\.profile$", "", idp_files)
...
# ── Save results ─────────────────────────────────────────────────────────────
out_dir <- file.path(CT_DIR, "association")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

fwrite(results, file.path(out_dir, "ct_association_results.csv"))
fwrite(meta,    file.path(out_dir, "ct_meta_results.csv"))

# Summary: top hits per outcome
top <- meta[order(p_meta)][, head(.SD, 20), by = outcome]
fwrite(top, file.path(out_dir, "ct_association_summary.csv"))
...
cat(sprintf("  ct_association_results.csv  (%d rows — per-cohort results)\n", nrow(results)))
cat(sprintf("  ct_meta_results.csv         (%d rows — meta-analysis)\n", nrow(meta)))
cat(sprintf("  ct_association_summary.csv  (top 20 per outcome)\n"))
